...
export const AdminSlides = () => {
  ...
  const handleDelete = async (id: number) => { // ← МЕНЯЕМ string на number
    if (confirm('Вы уверены, что хотите удалить этот слайд?')) {
      try {
        await slidesApi.deleteSlide(id); // ← Теперь id number
        await loadSlides();
      } catch (err) {
          ...
          <div key={slide.id} className={styles.slideCard}>
            ...
            <div className={styles.slideInfo}>
              ...
              <div className={styles.actions}>
                ...
                <button
                  // Преобразуем string в number для вызова handleDelete
                  onClick={() => handleDelete(Number(slide.id))} // ← ДОБАВЛЯЕМ Number()
